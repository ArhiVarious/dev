...
(function (d) {
    function j(b, a, c) {
        a = "(" + c.replace(k, "\\$1") + ")";
        return b.replace(RegExp(a, "gi"), "<strong>$1</strong>")
    }
    function g(b, a) {
        this.el = d(b);
    ...
    var k = RegExp("(\\/|\\.|\\*|\\+|\\?|\\||\\(|\\)|\\[|\\]|\\{|\\}|\\\\)", "g");
    d.fn.autocomplete = function (b, a) {
        var c;
        if ("string" == typeof b) {
            if (c = this.data("autocomplete"), "function" == typeof c[b]) c[b](a)
        } else c = new g(this.get(0) || d("<input />"), b);
        return c
    };    
    g.prototype = {
        killerFn: null, 
        initialize: function () {
            var b, a, c;
            b = this;
            a = Math.floor(1048576 * Math.random()).toString(16);
            c = "Autocomplete_" + a;
        ...
        extendOptions: function (b) {
            d.extend(this.options, b)
        }, 
        setOptions: function (b) {
            var a = this.options;
            this.extendOptions(b);
            if (a.lookup || a.isLocal) this.isLocal = !0, d.isArray(a.lookup) && (a.lookup = {
    ...
    getSuggestionsLocal: function (b) {
        ...
        b = b.toLowerCase();
        for (e = 0; e < d; e++) f = c.suggestions[e],
            0 === f.toLowerCase().indexOf(b) && (a.suggestions.push(f), a.data.push(c.data[e]));
        return a
    }, 
    getSuggestions: function (b) {
        var a, c;
        (a = this.isLocal ? this.getSuggestionsLocal(b) : this.cachedResponse[b]) && d.isArray(a.suggestions) ? (this.suggestions = a.suggestions, this.data = a.data, this.suggest()) : this.isBadQuery(b) || (c = this, c.options.params.query = b, d.get(this.serviceUrl, c.options.params, function (a) {
            c.processResponse(a)
        }, "text"))
    }, 
    ...
    suggest: function () {
        if (0 === this.suggestions.length) this.hide();
            ...
            h = this.options.fnFormatResult;
            f = this.getQuery(this.currentValue);
            g = function (a) {
                return function () {
                    b.activate(a)
                }
    ...
    for (e = 0; e < a; e++) c = this.suggestions[e], c = d((b.selectedIndex === e ? '<div class="selected"' : "<div") + ' title="' +
        c + '">' + h(c, this.data[e], f) + "</div>"), c.mouseover(g(e)), c.click(i(e)), this.container.append(c);
    this.enabled = !0;
    this.container.show()
} 
}, 
processResponse: function (b) {
    ...
    this.options.noCache || (this.cachedResponse[a.query] = a, 0 === a.suggestions.length && this.badQueries.push(a.query));
    a.query === this.getQuery(this.currentValue) && (this.suggestions = a.suggestions, this.data = a.data, this.suggest())
}, 
activate: function (b) {
    var a, c;
    a = this.container.children();
    -1 !== this.selectedIndex && a.length > this.selectedIndex && d(a.get(this.selectedIndex)).removeClass();
    this.selectedIndex = b;
    -1 !== this.selectedIndex && a.length > this.selectedIndex && (c = a.get(this.selectedIndex), d(c).addClass("selected"));
    return c
}, 
...
select: function (b) {
    var a;
    if (a = this.suggestions[b]) this.el.val(a), this.options.autoSubmit && (a = this.el.parents("form"), 0 < a.length && a.get(0).submit()), this.ignoreValueChange = !0,
        this.hide(), this.onSelect(b)
}, 
change: function (b) {
    if (this.suggestions[b]) {
        var a, c;
        a = this.suggestions[b];
        c = this.data[b];
        this.el.val(this.getValue(a))
    } else a = "", c = -1;
...
moveUp: function () {
    ...
    this.selectedIndex == -1 && this.adjustScroll(this.suggestions.length - 1)
}, 
moveDown: function () {
    this.adjustScroll(((this.selectedIndex == this.suggestions.length - 1)?-1:this.selectedIndex) +
        1)
}, 
adjustScroll: function (b) {
    ...
    a < c ? this.container.scrollTop(a) : a > d && this.container.scrollTop(a - this.options.maxHeight + 25);
...
getValue: function (b) {
    ...
    c = this.currentValue;
    a = c.split(a);
    return 1 === a.length ? b : c.substr(0, c.length - a[a.length - 1].length) + b
} 
}
})(jQuery);
